 ...
 * @version 02-26-2021
 */
public class RSBufferPool implements BufferPool {
    private RandomAccessFile disk;
    private MyLinkedList<Buffer> lru;
     ...
     * 
     * @param file
     *            A File that we are going to be reading from / writing to.
     * @param num_buffers
     *            The maximum number of buffers that we are allowed to
     *            keep loaded into main memory
     * @param block_size
     *            The size of an individual block.
     * @throws IOException
     */
    public RSBufferPool(File file, int num_buffers, int block_size)
        throws IOException {
        ...
        lru = new MyLinkedList<Buffer>(num_buffers);
     ...
     * 
     * @param block
     *            The index of the block we want to acquire.
    ...
    /**
     * Alert the RSBufferPool that a particular buffer was just used. If you
     * using RSBuffers then this should 'just happen' and require no
     * intervention on your part. This is where the LRU magic happens.
     * 
     * @param buffer
     *            The buffer to mark as just having been used.
     */
    ...
    /**
     * Alert the RSBufferPool that a particular buffer was just flushed. This
     * should then remove the buffer from the LRU.
     * 
     * @param buffer
     *            The buffer to mark as having been flushed.
     */
    ...
    /**
     * Allocate a new RSBuffer that is linked to this Buffer pool, knows its
     * size and offset, and has access to the RandomAccessFile.
     * 
     * @param block
     *            The block that this new buffer is backed by.
     * @return A handle to the newly created buffer.
     */
    private Buffer allocateNewBuffer(int block) {
        assert (block < max_buffers);
        return new RSBuffer(this, disk, block * block_size, block_size);
    }
